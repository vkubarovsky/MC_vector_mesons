...
!     ivec=1 rho, ivec=2 omega : exponential proposal exp(tslope*t)
!     ivec=3 phi,  ivec=4 jpsi : dipole proposal 1/(amg2-t)^4
!     For phi/jpsi this matches sigma_T_phi / sigma_T_jpsi exactly,
!     so the per-event t-weight is constant and acceptance ~ 100%.
!     NOTE: amg2 here MUST equal the parameter mg2 used inside
!           sigma_T_phi (line ~1249) and sigma_T_jpsi (line ~1316).
      tdmin_u = -tmax
      tdmax_u = -tmin
      if(ivec.le.2) then
!       Rho/omega: exponential exp(tslope*t)
        abslope = tslope
...
!       Phi/Jpsi: dipole 1/(amg2-t)^4 — matches phi/jpsi cross section
        amg2 = 1.6d0       ! matches mg2 in sigma_T_phi / sigma_T_jpsi
...
!     --- σ_T(W,Q²) parameters (same as phi except mass) ---
      parameter( alf1 = 400.d0  )
      parameter( alf2 =   1.0d0 )
      parameter( alf3 =   0.32d0)
      parameter( nuT  =   3.0d0 )
