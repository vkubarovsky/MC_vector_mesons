...
!
!     Transverse phi electroproduction cross section.
!     Model: sigma_T = AT * exp(bT*t) * xB/(1-xB) / Q2^3
!     (placeholder — same form as rho; update parameters to match data)
!
      implicit real*8(a-h,o-z)
!     --- tunable parameters ---
      parameter( AT = 15.d0  )   ! overall normalization
...
!     --------------------------
      ...
      sigma_T_phi = AT * exp(bT*t) * xB / (1d0-xB) / q2**3
      end

!----------------------------------------------------------------------
      real*8 function sigma_L_phi(q2,xB,t)
!
!     Longitudinal phi electroproduction cross section.
!     Model: sigma_L = AL * exp(bL*t) * xB / Q2^2
!     (placeholder — same form as rho; update parameters to match data)
!
      implicit real*8(a-h,o-z)
...
!     --------------------------
      ...
      sigma_L_phi = AL * exp(bL*t) * xB / q2**2
      end

!----------------------------------------------------------------------
      real*8 function sigma_T_jpsi(q2,xB,t)
!
!     Transverse J/psi electroproduction cross section.
!     Model: sigma_T = AT * exp(bT*t) * xB/(1-xB) / Q2^3
!     (placeholder — same form as rho; update parameters to match data)
!
      implicit real*8(a-h,o-z)
...
!     --------------------------
      ...
      sigma_T_jpsi = AT * exp(bT*t) * xB / (1d0-xB) / q2**3
      end

!----------------------------------------------------------------------
      real*8 function sigma_L_jpsi(q2,xB,t)
!
!     Longitudinal J/psi electroproduction cross section.
!     Model: sigma_L = AL * exp(bL*t) * xB / Q2^2
!     (placeholder — same form as rho; update parameters to match data)
!
      implicit real*8(a-h,o-z)
...
!     --------------------------
      ...
      sigma_L_jpsi = AL * exp(bL*t) * xB / q2**2
      end
