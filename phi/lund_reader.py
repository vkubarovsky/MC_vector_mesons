...
Particle identification by PID only — parent/status fields are ignored.

Logic:
  Beam electron  : always synthesized as k1 = [ebeam, 0, 0, ebeam] from header
  Target proton  : always synthesized as ptar = [Mp, 0, 0, 0]
  Scattered e-   : pid=11, lowest energy in event (beam is always highest)
  Recoil proton  : pid=2212, highest |p| in event (target is always at rest)
  pi+ / K+       : pid=211 / 321
  pi- / K-       : pid=-211 / -321
  Photon         : pid=22
  (Beam e- and target p in file are ignored via the selection rules above)

Beam energy from header tok[6].  Particle columns:
  p[0]=idx  p[1]=type  p[2]=type2  p[3]=PID  p[4]=parent  p[5]=parent2
