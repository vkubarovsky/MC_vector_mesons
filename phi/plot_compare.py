...
def fit_q2_powerlaw(ax, edges, counts, fit_range=(1.2, 4.5), color='red'):
...
def make_plot(d, vv, title, outfile, facecolor, edgecolor):
    ...
    # (1,3) phi mass
    ax = axes[1,3]
    ax.hist(d['Mphi'], bins=60, range=(0.97, 1.07), **bkw)
    ax.axvline(Mphi_meson, color='red', lw=1.5, ls='--', label=f'$M_\\phi$={Mphi_meson:.4f}')
    ax.set_xlabel(r'$M_{K^+K^-}$ (GeV)', fontsize=11)
    ax.set_ylabel('Events'); ax.set_title(r'$\phi$ invariant mass')
    ax.legend(fontsize=9)
