...
!
!     Akushevich/idiffrad VMD parameterization:
!       sigt = alpha_s^2 * Gamma_V * mV^3 / (3*alpha) * pi^3 * sk^2 * ff2
!       sigl = Q2/mV^2 * sigt
!     where sk encodes the VMD amplitude via pT^2 kinematics
!     ff2 = exp(tslope * t)  with tslope read from input file
!     ggam: photon-V coupling constants (rho, omega, phi, J/psi)
!
      implicit real*8(a-h,o-z)
      common/cmp/pi,alpha,amp,amp2,ap,ap2,aml2,amc2,amv,barn
      common/phi/phirad,tdif,phidif,tq,vmax,ivec
...
!               rho      omega    phi      J/psi
      ...
      sigt=al_s**2*ggam(ivec)*amv**3/3d0/alpha*pi**3*ask**2*aff2
      sigl=q2/amv2*sigt
      end
